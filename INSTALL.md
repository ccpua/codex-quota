...
向维护者获取完整的 `codex-quota` 源码文件夹并解压。当前项目没有配置可引用的远程仓库地址，所以本文不提供虚构的下载地址或 `git clone` 命令。
...
数据每 60 秒更新一次；电脑唤醒时也会尝试刷新。显示精度还取决于服务器更新时间。断网时会保留最后读数，并显示“待更新”或警告。重置时间使用当前电脑的本地时区。
