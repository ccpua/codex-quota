...
在右键或“…”菜单选择“语言 / Language → 中文 / English”可立即切换面板语言。选择“时区 / Time zone”可跟随系统或使用指定地区时区；两项设置都会保存，无需重启。福利预测原始时间按北京时间解析后转换，包含夏令时和跨日处理。额度重置时间及更新时间同样按所选时区显示。
