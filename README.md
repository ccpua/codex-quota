...
右键菜单或面板“…”菜单中可设置 **语言 / Language**（中文、English）和 **时区 / Time zone**。选择后面板立即更新，无需重启，设置会保存。时区默认跟随系统，也可按地区选择 IANA 时区（如 `Asia/Shanghai`、`America/Los_Angeles` 或 `UTC`）。
...
- 详情同时显示“下次福利重置（预测）”，时间来自远程配置，并显示本地倒计时。
